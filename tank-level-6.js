...
let TANK = {
  ...
  voltmeter: {
    id: 100,
    samples: new Array(CONFIG.samplesCount),
    idx: -1,
  ...
  notify: {
    ...
    send: 0,
    queuePopIdx: 0,
    queue: new Array(CONFIG.notify.queueCount),
  },
};
...
function sentNotifyTask() {
  ...
  if (
    CONFIG.uptime_ms - TANK.notify.lastSent_ms <
    CONFIG.notify.delaySec * 1000
  ) {
    if (TANK.notify.send >= CONFIG.notify.filter) {
      return;
    }
  } else {
    TANK.notify.send = 0;
  ...
  Shelly.call(
    "http.post",
    {
      url: CONFIG.notify.url + CONFIG.notify.whId,
    ...
    function (result, err_code, err_message) {
      if (err_code === 0) {
        TANK.notify.lastSent_ms = CONFIG.uptime_ms;
      }
