...
let TANK = {
  ...
  voltmeter: {
    id: 100,
    samples: new Array(CFG.samplesCount),
    idx: -1,
    fetchInProgerss: false,
  },
  level: { max: 5.63, min: 2.0, current: 0, pcnt: 0 },
  pumpThreshold: { low: 4.0, high: 5.2 }, // Water level threshold for stopping and starting the pump
  hydroThreshold: { low: 2.5, high: 3.0 }, // Water level threshold for stopping and starting the hydrophore
...
function sentNotifyTask() {
  ...
  if (CFG.uptime_ms - TANK.notify.lastSent_ms < CFG.notify.delaySec * 1000) {
    if (TANK.notify.send >= CFG.notify.filter) {
      return;
    }
  } else {
    TANK.notify.send = 0;
  ...
  Shelly.call(
    ...
    function (result, err_code, err_message) {
      if (err_code === 0) {
        TANK.notify.lastSent_ms = CFG.uptime_ms;
      }
...
function setValvesState(state) {
  ...
  TANK.shellyRemote[TANK.valves.shelly_id].switches[
    TANK.valves.sw_id
  ].desiredState = state;
...
function checkEmptyState() {
  ...
  for (let i = 0; i < TANK.shellyRemote.length; i++) {
    ...
    if (
      TANK.shellyRemote[i].fillRequest ||
      TANK.shellyRemote[i].fillRequestUser
    ) {
      if (!setValvesState(TANK.shellyRemote[i].valvesFillState)) {
        return true;
      }
      setPumpState(true);
...
function callShellyRemote(id) {
  ...
  for (let i = 0; i < TANK.shellyRemote[id].switches.length; i++) {
    ...
    body +=
      "sw" +
      TANK.shellyRemote[id].switches[i].id +
      "=" +
      TANK.shellyRemote[id].switches[i].desiredState;
  }
  Shelly.call(
    "http.get",
